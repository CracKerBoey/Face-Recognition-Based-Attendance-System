...
class Face_Register:
    ...
    def GUI_clear_data(self):
        ...
        if os.path.isdir(self.path_photos_from_camera):
            ...
            self.log_all["text"] = "No existing data to clear."
    ...
    def create_face_folder(self):
        ...
        if self.input_name_char:
            self.current_face_dir = os.path.join(self.path_photos_from_camera,
                                                  "person_" + str(self.existing_faces_cnt) + "_" + self.input_name_char)
        else:
            self.current_face_dir = os.path.join(self.path_photos_from_camera,
        ...
        if self.face_folder_created_flag:
            if self.current_frame_faces_cnt == 1:
                if not self.out_of_range_flag:
                    ...
                    cv2.imwrite(os.path.join(self.current_face_dir, f"img_face_{self.ss_cnt}.jpg"),
                                self.face_ROI_image)
    ...
    def check_existing_faces_cnt(self):
        if os.path.isdir("data/data_faces_from_camera/"):
